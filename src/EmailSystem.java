...
public class EmailSystem {
    ...
    private static void forgotMail() {
       int id = Integer.parseInt(input("""
                  Step 1
         Enter your Account ID"""));
       String lastName = input("what is your last Name");
       try {
            User user = userController.findById(id,lastName);
            display(user.getEmail());
       }
       catch (IllegalArgumentException e){display("""
               invalid Id Or LastName please enter the correct id and\040
               lastName to help us retrieve your account"""); startApp();}
    ...
    private static void deleteUser() {
         try {
             ...
             userController.deleteUser(id); String comment = input("""
                     please leave a comment on why you are deleting your account""");
             display("""
                      we are very sorry we will try to fix that
                     """
                        +comment);startApp();
         }
    ...
    private static void sendMail(){
        ...
        display("sent");
        User user = userController.findByEmail(recipientEMail);
